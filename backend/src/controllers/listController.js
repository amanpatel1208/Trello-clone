...
exports.updateList = async (req, res) => {
  ...
  const r = await db.query(
    'UPDATE lists SET title=COALESCE($1,title), archived=COALESCE($2,archived), color=COALESCE($3,color) WHERE id=$4 RETURNING *',
    [title?.trim(), archived, color, req.params.id]
  );
