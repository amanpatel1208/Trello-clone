const BASE = (import.meta.env.VITE_API_URL || 'https://trello-clone-lwq4.onrender.com') + '/api';

const req = async (url, opts = {}) => {
  const r = await fetch(BASE + url, { headers: { 'Content-Type': 'application/json' }, cache: 'no-store', ...opts });
...
export const getBoard = (id, memberId) => req(`/boards/${id}${memberId ? `?member_id=${memberId}` : ''}`);
...
export const toggleStarBoard = (id, memberId) => req(`/boards/${id}/toggle-star`, { method: 'POST', body: JSON.stringify({ member_id: memberId }) });
export const getBoardActivity = (id, memberId) => req(`/boards/${id}/activity${memberId ? `?member_id=${memberId}` : ''}`);
